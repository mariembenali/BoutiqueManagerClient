...
public class MainViewController {
	...
	void save_user(ActionEvent event) throws Exception {
		if (!txtIdUser.getText().trim().isEmpty() && !txtAdresseUser.getText().trim().isEmpty()
				&& !txtNomUser.getText().trim().isEmpty() && !txtPrenomUser.getText().trim().isEmpty()
				&& !txtEmailUser.getText().trim().isEmpty()
				&& cbxProfilUser.getSelectionModel().getSelectedItem() != null) {
			User u = new User();
	...
	void add_commande(ActionEvent event) {
		if (cbxProduitComm.getSelectionModel().getSelectedItem().getQuantite() >= Integer
				.parseInt(txtQuantiteComm.getText())) {
			boolean trouve = false;
			int position = 0;
			for (Detail detail : listeCommande) {
				if (detail.getProduit().getId() == cbxProduitComm.getSelectionModel().getSelectedItem().getId()) {
					trouve = true;
					if ((detail.getQuantite() + Integer.parseInt(txtQuantiteComm.getText())) < cbxProduitComm
							.getSelectionModel().getSelectedItem().getQuantite()) {
						listeCommande.get(position)
								.setQuantite(detail.getQuantite() + Integer.parseInt(txtQuantiteComm.getText()));
						listeCommande.get(position).setTotal(
								detail.getProduit().getPrixUnitaire() * listeCommande.get(position).getQuantite());
						tableListeCommande.getItems().clear();
						try {
							Fabrique.getiProduit()
									.findProduit(cbxProduitComm.getSelectionModel().getSelectedItem().getId())
									.setQuantite(cbxProduitComm.getSelectionModel().getSelectedItem().getQuantite()
											- detail.getQuantite());
						} catch (Exception e) {
							e.printStackTrace();
						}
						...
						txtQuantiteComm.setText(
								String.valueOf(cbxProduitComm.getSelectionModel().getSelectedItem().getQuantite()
										- detail.getQuantite()));
	...
	void supprimer_detail(ActionEvent event) {
		if (listeCommande.size() > 0) {
			listeCommande.removeIf(d -> d.getProduit().getLibelle()
					.equals(tableListeCommande.getSelectionModel().getSelectedItem().getProduit().getLibelle()));
		}
	...
	void save_commande(ActionEvent event) throws Exception {
		...
		if (!txtIdClient.getText().trim().isEmpty() && !txtNomClient.getText().trim().isEmpty()
				&& txtDateNaissance.getValue() != null && !txtAdresse.getText().trim().equals("")
				&& !txtEmail.getText().trim().equals("") && listeCommande.size() > 0
			...
			if (client == null) {
				...
				if (cbxTypeClient.getSelectionModel().getSelectedItem().getLibelle().equals("Particulier")
						&& !txtPrenomClient.getText().trim().isEmpty())
	...
	void initialize() throws Exception {
		...
		forceInputNumber(txtMarge); // forcer type de input as int
	...
	private void actualiserTables() throws Exception {
		clnCategorieProd.setCellValueFactory(
				cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getCategorie().getLibelle()));
		clnLibelleProduit
				.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getLibelle()));
		clnPrixProduit
				.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getPrixUnitaire()));
		clnQuantiteProduit
				.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getQuantite()));
		...
		clnDateComm
				.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getDateLivraison()));
		...
		clnClientCommande.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(
				cellData.getValue().getClient().getPrenom() + " " + cellData.getValue().getClient().getNom()));
		...
		clnDateCommFact
				.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getDateLivraison()));
		clnAcompteCommFact
				.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getAcompte()));
		clnMontantCommFact
				.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getMontant()));
		clnClientCommandeFact.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(
				cellData.getValue().getClient().getPrenom() + " " + cellData.getValue().getClient().getNom()));
		...
		clnUserProfil.setCellValueFactory(
				cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getProfil().getLibelle()));
	...
	private void actualiserCommListe(List<Detail> liste) {
		clnProduitDetail.setCellValueFactory(
				cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getProduit().getLibelle()));
		clnQuantiteDetail
				.setCellValueFactory(cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getQuantite()));
		clnPrixDetail.setCellValueFactory(
				cellData -> new ReadOnlyObjectWrapper<>(cellData.getValue().getProduit().getPrixUnitaire()));
	...
	public static void generate_bon(Commande c) {
		...
		try {
			...
			PdfWriter writer = PdfWriter.getInstance(document,
					new FileOutputStream("Documents/BonDeCommande/" + c.getNumero() + ".pdf"));
			document.open();
			...
			document.add(img);
			Paragraph p = new Paragraph(LocalDate.now().format(DateTimeFormatter.ofPattern("dd - MM - yyyy")),
					new Font(Font.FontFamily.TIMES_ROMAN, 16f));
			p.setAlignment(2);
			document.add(p);
			...
			PdfPCell cell = new PdfPCell(
					new Paragraph("Designation", new Font(Font.FontFamily.TIMES_ROMAN, 14f, Font.BOLD)));
			PdfPCell cell1 = new PdfPCell(
					new Paragraph("Quantité", new Font(Font.FontFamily.TIMES_ROMAN, 14f, Font.BOLD)));
			PdfPCell cell2 = new PdfPCell(
					new Paragraph("Prix Unitaire", new Font(Font.FontFamily.TIMES_ROMAN, 14f, Font.BOLD)));
			PdfPCell cell3 = new PdfPCell(
					new Paragraph("Total", new Font(Font.FontFamily.TIMES_ROMAN, 14f, Font.BOLD)));
			...
			for (Detail detail : c.getDetails()) {
				PdfPCell c1 = new PdfPCell(
						new Paragraph(detail.getProduit().getLibelle(), new Font(Font.FontFamily.TIMES_ROMAN, 14f)));
				PdfPCell c2 = new PdfPCell(new Paragraph(String.valueOf(detail.getQuantite()),
						new Font(Font.FontFamily.TIMES_ROMAN, 14f)));
				PdfPCell c3 = new PdfPCell(new Paragraph(String.valueOf(detail.getProduit().getPrixUnitaire()),
						new Font(Font.FontFamily.TIMES_ROMAN, 14f)));
				PdfPCell c4 = new PdfPCell(
						new Paragraph(String.valueOf(detail.getTotal()), new Font(Font.FontFamily.TIMES_ROMAN, 14f)));
	...
	public static void generate_facture(Facture f) {
		...
		try {
			...
			PdfWriter writer = PdfWriter.getInstance(document,
					new FileOutputStream("C:\\Users\\ghada\\Desktop\\" + f.getNumero() + ".pdf"));
			document.open();
			...
			if (f.getClient().getType().getLibelle().equals("Entreprise") && f.getCommande().getEtat() == 0) {
				Paragraph foot1 = new Paragraph("Montant Perçu :",
						new Font(Font.FontFamily.TIMES_ROMAN, 16f, Font.BOLD));
